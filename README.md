...
Проект готов к развёртыванию в контейнере:

```bash
cp .env.example .env
# DATABASE_URL=sqlite+aiosqlite:////data/ghosthub.db
# STORAGE_PATH=/data/storage
docker compose up -d --build
```

Миграции выполняются автоматически при старте контейнера, данные (БД и файлы)
хранятся в volume `ghosthub_data`.
