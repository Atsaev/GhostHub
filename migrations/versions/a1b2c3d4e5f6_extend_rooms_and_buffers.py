...
down_revision: Union[str, Sequence[str], None] = '8902e3771e0a'
...
def upgrade() -> None:
    """Upgrade schema."""
    op.add_column('rooms', sa.Column('password_hash', sa.String(length=128), nullable=True))
    op.create_index(op.f('ix_rooms_expires_at'), 'rooms', ['expires_at'], unique=False)

    op.add_column('buffers', sa.Column('kind', sa.String(length=8), server_default='text', nullable=False))
    op.add_column('buffers', sa.Column('file_name', sa.String(length=255), nullable=True))
    op.add_column('buffers', sa.Column('file_size', sa.BigInteger(), nullable=True))
    op.add_column('buffers', sa.Column('mime_type', sa.String(length=128), nullable=True))
    op.add_column('buffers', sa.Column('device_id', sa.String(length=64), server_default='unknown', nullable=False))
    op.create_index(op.f('ix_buffers_room_id'), 'buffers', ['room_id'], unique=False)
    op.create_index(op.f('ix_buffers_device_id'), 'buffers', ['device_id'], unique=False)
